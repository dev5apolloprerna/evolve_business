...
class CheckApprovalStatus
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($user->role_id == 2) {
            ...
            if (
                $loginPendingCheck->isNotEmpty() ||
                $loginPendingOneToOneCheck->isNotEmpty() ||
                $loginPendingEventCheck->isNotEmpty() ||
                $Member_metting->isNotEmpty() ||
                $pendingMeeting->isNotEmpty()
            ) {
